...
public:
    ...
    //These are convenience functions that make it easy to setup a whole set of parameters
    void setSearchScale(double fov_low, double fov_high, QString units);    //This sets the scale range for the image to speed up the solver
    void setSearchPosition(double ra, double dec, double rad);              //This sets the search RA/DEC/Radius to speed up the solver
    void createConvFilterFromFWHM(double fwhm);                             //This creates the conv filter from a fwhm
...
protected:  //Note: These items are not private because they are needed by ExternalSextractorSolver
    ...
    //This is the filter used for convolution
