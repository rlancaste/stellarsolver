...
int MainWindow::resort_xylist(const char* infn, const char* outfn,
    ...
    fin = fopen(infn, "rb");
    if (!fin) {
        emit logNeedsUpdating(QString("Failed to open input file %s").arg(infn));
        return -1;
    }

    fout = fopen(outfn, "wb");
    if (!fout) {
        emit logNeedsUpdating(QString("Failed to open output file %s").arg(outfn));
        goto bailout;
    }
    ...
    if (!anq) {
        emit logNeedsUpdating(QString("Failed to open file \"%s\"").arg(infn));
        goto bailout;
    }
    ...
    if (pipe_file_offset(fin, start, size, fout)) {
        emit logNeedsUpdating("Failed to copy primary FITS header.");
        goto bailout;
    }
    ...
    if (!tab) {
        emit logNeedsUpdating(QString("Failed to open FITS table in file %s").arg(infn));
        goto bailout;
    }
    ...
    for (ext=1; ext<nextens; ext++) {
        ...
        if (!anqfits_is_table(anq, ext)) {
            emit logNeedsUpdating(QString("Extension %i isn't a table. Skipping").arg(ext));
            continue;
        }
        // Copy the header as-is.
        if (pipe_file_offset(fin, hdrstart, hdrsize, fout)) {
            emit logNeedsUpdating(QString("Failed to copy the header of extension %i").arg(ext));
            goto bailout;
        }

        if (fitstable_read_extension(tab, ext)) {
            emit logNeedsUpdating(QString("Failed to read FITS table from extension %i").arg(ext));
            goto bailout;
        }
        rowsize = fitstable_row_size(tab);

        // read FLUX column as doubles.
        flux = (double *) fitstable_read_column(tab, fluxcol, TFITS_BIN_TYPE_D);
        if (!flux) {
            emit logNeedsUpdating(QString("Failed to read FLUX column from extension %i").arg(ext));
            goto bailout;
        }
        // BACKGROUND
        back = (double *) fitstable_read_column(tab, backcol, TFITS_BIN_TYPE_D);
        if (!back) {
            emit logNeedsUpdating(QString("Failed to read BACKGROUND column from extension %i").arg(ext));
            goto bailout;
        }

        N = fitstable_nrows(tab);
        logNeedsUpdating(QString("First rows of input table:\n"));
        for (i=0; i<MIN(10, N); i++)
            logNeedsUpdating(QString("flux %g, background %g\n").arg(flux[i]).arg( back[i]));
        ...
        for (i=0; i<N; i++) {
            ...
            for (j=0; j<2; j++) {
                ...
                if (used[index])
                    continue;
                used[index] = TRUE;
                //emit logNeedsUpdating(QString("adding index %i: %s %g\n").arg(index, j==0 ? "flux" : "bgsub").arg(j==0 ? flux[index] : back[index]));
                if (pipe_file_offset(fin, datstart + index * rowsize, rowsize, fout)) {
                    emit logNeedsUpdating(QString("Failed to copy row %i").arg(index));
                    goto bailout;
                }
            }
        }
        ...
        if (fits_pad_file(fout)) {
            emit logNeedsUpdating(QString("Failed to add padding to extension %i").arg(ext));
            goto bailout;
        }
    ...
    if (fclose(fout)) {
        emit logNeedsUpdating(QString("Failed to close output file %s").arg(outfn));
        return -1;
    }
    fclose(fin);
    return 0;
...
int MainWindow::tabsort(const char* infn, const char* outfn, const char* colname,
    ...
    anq = anqfits_open(infn);
    if (!anq) {
        emit logNeedsUpdating(QString("Failed to open \"%s\"").arg(infn));
        goto bailout;
    }
    start = anqfits_header_start(anq, 0);
    size  = anqfits_header_size (anq, 0);
    if (pipe_file_offset(fin, start, size, fout)) {
        emit logNeedsUpdating(QString("Failed to copy primary FITS header."));
        goto bailout;
    }
    ...
    for (ext=1; ext<nextens; ext++) {
        ...
        if (!anqfits_is_table(anq, ext)) {
            emit logNeedsUpdating(QString("Extension %i isn't a table. Skipping.\n").arg(ext));
            continue;
        }
        table = anqfits_get_table(anq, ext);
        if (!table) {
            emit logNeedsUpdating(QString("Failed to open table: file %s, extension %i. Skipping.").arg(infn, ext));
            continue;
        }
        c = fits_find_column(table, colname);
        if (c == -1) {
            emit logNeedsUpdating(QString("Couldn't find column named \"%s\" in extension %i.  Skipping.").arg(colname, ext));
            continue;
        }
        col = table->col + c;
        ...
        default:
            emit logNeedsUpdating(QString("Column %s is neither FITS type D, E, nor K.  Skipping.").arg(colname));
            continue;
        }
        ...
        qfits_query_column_seq_to_array(table, c, 0, table->nr, (uchar*) data, atomsize);
        // Sort it.
        ...
        mapsize = msize;
        map = (uchar *) mmap(NULL, mapsize, PROT_READ, MAP_SHARED, fileno(fin), mstart);
        if (map == MAP_FAILED) {
            emit logNeedsUpdating(QString("Failed to mmap input file %s").arg(infn));
            map = NULL;
            goto bailout;
        }
        ...
        if (fwrite(tablehdr, 1, hdrsize, fout) != hdrsize) {
            emit logNeedsUpdating(QString("Failed to write FITS table header"));
            goto bailout;
        }
        ...
        for (i=0; i<table->nr; i++) {
            ...
            if (fwrite(rowptr, 1, table->tab_w, fout) != table->tab_w) {
                emit logNeedsUpdating(QString("Failed to write FITS table row"));
                goto bailout;
            }
        }
    ...
    if (fclose(fout)) {
        emit logNeedsUpdating(QString("Error closing output file"));
        fout = NULL;
        goto bailout;
    }
    fclose(fin);
