...
CommandLineParseResult parseCommandLine(QCommandLineParser &parser, StellarSolverCliQuery *query)
{
    ...
    parser.addOptions({
        ...
        {{"l", "cpulimit"},
         ...
         "seconds"},
         {"index-files",
         "Specifies the directory where the index files are located (default: xy)",
         "path"} 
        // TODO Options to overwrite profile, allow parsing of astrometry.cfg, output file
    });
...
void solve(QString image_file, StellarSolverCliQuery *query)
{
    
    fileio imageLoader;
    imageLoader.logToSignal = false;
    if (!imageLoader.loadImage(image_file))
    { //TODO this should throw an error here
